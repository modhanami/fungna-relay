...
function setupSocketIoPeerConnectionEventHandlers(pc: RTCPeerConnection, transport: RTCTransportSender) {
...
async function makeOffer(spc: SocketIoPeerConnection, audioDeviceId = 'default') {
  ...
  for (const track of localStream.getTracks()) {
    const sender = spc.pc.getSenders().find(sender => sender.track.kind === track.kind);
    if (sender) {
